...
void camera_transform(camera_t *camera, T3DViewport* viewport)
{
    ...
    memcpy(&camera->forward, &forward, sizeof(fm_vec3_t));
    fm_vec3_lerp(&camera->camTarget_off, &camera->camTarget_off, &forward, 0.2f);
    
    fm_vec3_t position = camera->position;
    fm_vec3_scale(&position, &position, T3D_TOUNITSCALE);
    fm_vec3_lerp(&camera->camPos_off, &camera->camPos_off, &position, 0.2f);

    fm_vec3_add(&forward, &camera->camPos_off, &camera->camTarget_off);
    
    t3d_viewport_set_perspective(viewport, T3D_DEG_TO_RAD(camera->FOV), camera->aspect_ratio, camera->near_plane, camera->far_plane);
    t3d_viewport_look_at(viewport, (T3DVec3*)&camera->camPos_off, (T3DVec3*)&forward, &(T3DVec3){{0,1,0}});
}